...
a.lastFinished=!0;b.data("validation"+a.validatorName+"Message",a.message);setTimeout(function(){b.trigger("revalidate.validation")},1)}}));return!1}},regex:{name:"regex",init:function(b,c){var a={},d=b.data("validation"+c+"Regex");a.regex=RegExp("^"+d+"$");void 0===d&&f.error("Can't find regex for '"+c+"' validator on '"+b.attr("name")+"'");d="Not in the expected format";b.data("validation"+c+"Message")&&(d=b.data("validation"+c+"Message"));a.message=d;a.originalName=c;return a},validate:function(b,
...
a.minchecked&&a.negative},blockSubmit:!0,includeEmpty:!0},number:{name:"number",init:function(b,c){var a={step:1};b.attr("step")&&(a.step=b.attr("step"));b.data("validation"+c+"Step")&&(a.step=b.data("validation"+c+"Step"));a.decimal=".";b.data("validation"+c+"Decimal")&&(a.decimal=b.data("validation"+c+"Decimal"));a.thousands="";b.data("validation"+c+"Thousands")&&(a.thousands=b.data("validation"+c+"Thousands"));a.regex=RegExp("^"+("([+-]?\\d+(\\"+a.decimal+"\\d+)?)?")+"$");a.message="Must be a number";
...
b.parents("form").first()||b.parents(".control-group").first())&&(c=a.find("input[name='"+b.attr("name")+"']:checked").map(function(a,b){return f(b).val()}).toArray().join(","))):c="number"===a?b[0].validity.valid?b.val():b[0].validity.badInput||b[0].validity.stepMismatch?"NaN":"":b.val();return c};f.fn.jqBootstrapValidation=function(b){return m.methods[b]?m.methods[b].apply(this,Array.prototype.slice.call(arguments,1)):"object"!==typeof b&&b?(f.error("Method "+b+" does not exist on jQuery.jqBootstrapValidation"),
